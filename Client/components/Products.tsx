...
const ProductCard = ({ product }:{ product:Product }) => {
  ...
  return (
    <div
      className='relative flex flex-col border border-slate-200 bg-white rounded-2xl lg:max-h-[410px] sm:max-w-[220px] p-2 overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl'
    ...
    >
      ...
      <div className={`relative transition-transform mb-1 duration-300 ${isHovered && 'scale-105'}`}>
        <img className='min-w-[200px] min-h-[210px] rounded-xl bg-slate-50 object-cover' src={product.images.imglink} alt={product.title} />
        {isHovered && (
          <button
      ...
      <div className='pl-4 pr-4 flex flex-col gap-2'>
        ...
        <Link onClick={()=>trackEvent({eventName:'product_viewed',productId:product.productid,metadata:{source:'home_product_card',category:product.category}})} href={`/product/${product.productid}`}><p className='tracking-[1px] text-silver hover:text-davysilver'>{product.title}</p></Link>
...
const Products = () => {
  ...
  return (
    <div className='sm:ml-4 ml-auto mr-auto pb-8 max-w-[980px] flex flex-col flex-1'>
      <div className='border-b border-slate-200 pb-3'><p className='text-xs font-bold uppercase tracking-[.18em] text-emerald-700'>Curated discovery</p><p className='mt-1 text-2xl font-bold text-slate-950'>Popular products</p></div>
