...
import { checkoutProductDataHandler, createDemoOrderHandler } from '@/app/api/paymentSystem';
...
const Checkout = () => {
    ...
    async function createOrder(e:any){
        e.preventDefault();
        setloading(true);
        const createOrder = await createDemoOrderHandler({userid:genUserData.current.userID,productid:params.productID,colorid:params.colorID,sizeid:params.sizeID})
    ...
    <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
        ...
        <form action="/" id='informational-form' onSubmit={createOrder} className="min-w-0 flex-1 space-y-8">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="flex items-center">
                    <span className="z-10 inline-flex shrink-0 items-center rounded-s-lg border border-gray-300 bg-gray-100 px-3 py-2.5 text-sm font-medium text-gray-600">Phone</span>
            ...
            <div className="space-y-3 rounded-xl border border-emerald-200 bg-emerald-50 p-5">
              <h3 className="text-xl font-semibold text-gray-900">Portfolio Demo Checkout</h3>
              <p className="text-sm text-gray-600">No real card or payment gateway is used. Placing the order records a demo order so the end-to-end shopping journey remains testable.</p>
            </div>
            ...
            <div>
            <label htmlFor="voucher" className="mb-2 block text-sm font-medium text-gray-900 dark:text-white"> Enter a promotional code (demo) </label>
        ...
        <div className="mt-6 w-full space-y-6 sm:mt-8 lg:mt-0 lg:max-w-xs xl:max-w-md">
            ...
            <div className="space-y-3">
              <button form='informational-form' disabled={!loggedIn || loading} type="submit" className="flex w-full items-center justify-center rounded-lg bg-primary-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-primary-800 disabled:opacity-50">Place Demo Order</button>
              <p className="text-xs text-gray-500">Portfolio mode: no real payment is collected.</p>
            </div>
        </div>
        </div>
    </div>
