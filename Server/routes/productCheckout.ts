...
import {orderCreationSchema,checkoutSchema,OrderIDSchema} from '../validators/productCheckoutValidator';
...
router.post('/demo-checkout/create-order',orderCreationSchema, async (req:Request, res:Response) => {
    ...
    try {
      ...
      try {
        ...
        await conn.query(
          ...
          [paymentid, orderid, 'Portfolio Demo', 'Not Applicable', amount, transactionid, addressid]
        );
        await conn.query(
...
router.get('/orders/status/:orderID',OrderIDSchema, async (req:Request, res:Response) => {
  ...
  if(result.isEmpty()){
    ...
    try {
      ...
      if (orderstatus === 'Confirmed' && paymentmethod === 'Portfolio Demo') {
